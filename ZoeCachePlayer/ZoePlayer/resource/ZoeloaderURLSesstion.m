...
@interface ZoeloaderURLSesstion ()<VideoRequestTaskDelegate>
...
@implementation ZoeloaderURLSesstion
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (void)dealWithLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest
{
    ...
    if (!self.task) {
        self.task = [[ZoeViderRequestTask alloc] init];
